<template>
  ...
  <BaseDialog
    v-model="DialogAdd"
    title="Thêm dữ liệu sử dụng phụ tùng"
    max-width="500"
    icon="mdi-plus"
  >
    <v-row>
  ...
  <BaseDialog
    ...
    icon="mdi-delete"
    title="Xóa dữ liệu sử dụng phụ tùng"
    max-width="500"
  >
...
const SaveEdit = async () => {
  ...
  try {
    const response = await axios.put(
      `${Url}/Maintenance/SparePartUsage/Edit-item/${GetID.value}`,
      formData,
    );
...
const SaveAdd = async () => {
  ...
  try {
    const response = await axios.post(
      `${Url}/Maintenance/SparePartUsage/Add-item`,
      formData,
    );
...
const RemoveItem = async () => {
  ...
  try {
    const response = await axios.delete(
      `${Url}/Maintenance/SparePartUsage/Delete-item/${GetID.value}`,
    );
    console.log(response.data.message);
