<template lang="">
        ...
        >
          ...
          >
            <v-badge
              ...
              :content="
                getAccessories(internalItem.raw)[0].id === null
                  ? 0
                  : getAccessories(internalItem.raw).length
              "
              :color="
                getAccessories(internalItem.raw)[0].id === null
                  ? 'grey'
  ...
  >
    <v-row>
      ...
      <v-col cols="6">
        <InputField
          v-model.number="TempAccessory.SoLuongSuDung"
          type="number"
          label="Số lượng"
        />
      </v-col>
...
const getAccessories = (item) => {
  ...
  try {
    ...
    return Array.isArray(data)
      ? data.filter((i) => i && (i.id || i.TenPhuTung))
      : [];
  } catch (e) {
...
const RemoveAccessoryRowEdit = async (index, accessoryId) => {
  ...
  if (accessoryId) {
    try {
      await axios.delete(
        `${Url}/Maintenance/SparePart/Delete-item/${accessoryId}`,
      );
    } catch (error) {
  ...
  try {
    ...
    for (const acc of Accessory_Edit.value) {
      ...
      if (acc.MaSuDung || acc.id) {
        ...
        await axios.post(`${Url}/Maintenance/SparePart/Add-item`, payload);
      }
    }
...
const SaveAdd = async () => {
  ...
  try {
    ...
    for (const acc of Accessory.value) {
      await axios.post(`${Url}/Maintenance/SparePart/Add-item`, {
