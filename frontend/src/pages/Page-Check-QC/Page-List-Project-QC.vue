        ...
        >
          <template v-slot:bottom>
            <div class="text-center pt-2">
              <v-pagination
                v-model="page"
                :length="Math.ceil(filterBomQC.length / itemsPerPage)"
              ></v-pagination>
            </div>
          </template>
...
const { filterBomQC, filterBomQCError } = useFilterBomQC();
...
function PushItem(value) {
  router.push(`/Kiem-tra-pnp-qc/${value}`);
  const found = filterBomQC.value.find((v) => v.id === value);
  localStorage.setItem("ProjectQC", found.project_name);
}
...
const SaveAdd = async () => {
  ...
  try {
    // 👉 1. tạo FilterBomQC
    const res1 = await axios.post(`${Url}/FilterBomQC/Add-item`, formData);
...
const SaveEdit = async () => {
  ...
  try {
    const response = await axios.put(
      `${Url}/FilterBomQC/Edit-item/${GetID.value}`,
      formData,
    );
    Reset();
    MessageDialog.value = "Thêm dữ liệu dự án thành công";
...
const RemoveItem = async () => {
  ...
  try {
    const response = await axios.delete(
      `${Url}/FilterBomQC/Delete-item/${GetID.value}`,
    );
