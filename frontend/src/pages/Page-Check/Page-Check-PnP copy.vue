                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <!-- Lớp Gerber (Tĩnh) -->
                        <g
                          v-html="gerberInnerContent"
                          class="gerber-bg-layer viewport"
                        ></g>
                        ...
                        >
                          ...
                          >
                            ...
                            <polygon
                              ...
                              fill="#D32F2F"
                              :transform="`translate(${marker.arrowOffset}, 0)`"
                            />
...
// Trích xuất nội dung bên trong <svg>...</svg> để render tách biệt
const gerberInnerContent = computed(() => {
  const svg = currentGerberSvg.value;
  if (!svg) return "";
  // Tìm nội dung giữa thẻ <svg ...> và </svg>
  const match = svg.match(/<svg[^>]*>([\s\S]*)<\/svg>/i);
  return match ? match[1] : "";
});

// Dữ liệu marker đã được tính toán sẵn để render mượt mà bằng v-for
const pnpMarkersArray = computed(() => {
  ...
  return filteredPnP.value
    ...
    .map((pnp) => {
      ...
      let displayRotation = isBottom
        ? (180 - pnpRot - uiRot) % 360
        : (180 - pnpRot + uiRot) % 360;
      if (displayRotation < 0) displayRotation += 360;

      const finalTx = tx + OFFSET_X * factor;
      const finalTy = ty + OFFSET_Y * factor;
      ...
      return {
        ...
        textTransform: `${textFlip} rotate(${-displayRotation})`,
        rectW,
        rectL,
        strokeMain,
        anchorSize,
        ...
        arrowSize,
        arrowOffset,
        arrowPoints: `0,0 -${arrowSize},${-arrowSize / 2} -${arrowSize},${
          arrowSize / 2
        }`,
      };
    });
...
// Combine Gerber + Pick&Place overlay (Giữ lại tên cũ để tránh lỗi tham chiếu nếu có, nhưng giá trị là null)
const svgWithPnP = ref(null);
...
function getCachedSvgEl() {
  if (!_cachedSvgEl || !_cachedSvgEl.isConnected) {
    _cachedSvgEl = svgWrapper.value ?? null;
  }
  return _cachedSvgEl;
}
...
// Theo dõi khi GERBER thay đổi để khởi tạo baseViewBox ban đầu
watch(
  () => currentGerberSvg.value,
  (newSvg) => {
    if (!newSvg) return;
...
watch(currentGerberSvg, () => {
  nextTick(() => {
    applySelectionHighlights();
  });
});

// Thêm watcher cho markers array để đảm bảo highlight vẫn đúng khi markers thay đổi (vd chuyển layer)
...
.gerber-svg-container-full :deep(.pnp-marker) {
  cursor: pointer;
  z-index: 999;
  /* Giúp browser tối ưu việc repaint */
...
.pnp-marker {
  /* KHÔNG dùng transition trên marker khi zoom/pan – gây lag cực nặng */
  pointer-events: auto;
}
