            ...
            <template #[`bottom`]>
              <div class="text-center pt-2">
                <v-pagination
                  v-model="page"
                  :length="
                    Math.ceil((manufactureAssembly?.length || 0) / itemsPerPage)
                  "
                ></v-pagination>
              </div>
            </template>
...
const { manufactureAssembly, manufactureAssemblyError } =
  useManufactureAssembly(id);
...
watch(
  manufactureAssembly,
  (newValue) => {
    console.log("manufactureAssembly updated:", newValue);
