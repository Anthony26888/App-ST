            ...
            <v-card class="rounded-lg" color="success" variant="tonal">
              <v-card-text>
                ...
                <div class="text-caption">
                  Tổng số lượng đầu ra ( {{ Quantity_IPQCSMT }} pcs/ panel)
                </div>
              </v-card-text>
            </v-card>
          </v-col>
...
import { useManufactureIPQCSMT } from "@/composables/Manufacture/useManufactureIPQC-SMT";
...
watch(manufactureIPQCSMTError, (error) => {
  if (error) {
    console.error("ManufactureIPQCSMT Error:", error);
  }
});
